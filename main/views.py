...
def send_payment_receipt(enrollment):
    ...
    # -----------------------------
    subject = "Enrolment Payment Successful – Receipt"
    message = (
        f"Dear {enrollment.full_name},\n\n"
...
def profile_view(request):
    ...
    # Redirect if payment not verified
        if not verified_payment and not enrollment.is_course_activated:
            messages.warning(
                request,
                "Access denied: Please complete your course payment or wait for admin confirmation if you have paid through Bank transfer."
            )
