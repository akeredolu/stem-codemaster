...
def our_services(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # ✅ FIXED: Changed parameter name from plain_text to html_content
            send_brevo_email(
...
def payment_page(request, service_request_id):
    """Separate payment page for service requests with safe status routing."""
    # ✅ FIX: Fetch purely by ID so reverse lookups do not crash when status updates to 'paid'
    service_request = get_object_or_404(ServiceRequest, id=service_request_id)
    
    # Optional safety check: If already paid, send them directly to the pending/success screen
